				   ...
				   int size)
{
	...
	if (pattern == 0x09006c || pattern == 0x09006d || pattern == 0x080007) {
		...
		switch (data[3]) {
		case 0x6C:
			zotac_gamepad_send_button(zotac, (int[]){ BTN_MODE },
						  1);
			break;
		case 0x6D:
...
static int zotac_probe(struct hid_device *hdev, const struct hid_device_id *id)
{
	...
	zotac = devm_kzalloc(&hdev->dev, sizeof(*zotac), GFP_KERNEL);
	if (!zotac)
		return -ENOMEM;
...
{
	struct zotac_device *zotac = hid_get_drvdata(hdev);
	struct zotac_gamepad *gamepad = zotac ? zotac->gamepad : NULL;
	int intf_num = get_interface_num(hdev);
	int ret = 0;

	if (gamepad && gamepad->urbs[0] &&
	    (intf_num == ZOTAC_GAMEPAD_INTERFACE ||
	     intf_num == ZOTAC_DIAL_INTERFACE)) {
		ret = usb_submit_urb(gamepad->urbs[0], GFP_NOIO);
		if (ret) {
...
static int zotac_reset_resume(struct hid_device *hdev)
{
	...
	if (zotac && zotac->led_rgb_dev && intf_num == ZOTAC_COMMAND_INTERFACE)
...
static int zotac_suspend(struct hid_device *hdev, pm_message_t message)
{
	...
	if (gamepad && (intf_num == ZOTAC_GAMEPAD_INTERFACE ||
			intf_num == ZOTAC_DIAL_INTERFACE)) {
		/* Kill all input URBs */
...
static void zotac_remove(struct hid_device *hdev)
{
	...
	if (zotac && intf_num == ZOTAC_COMMAND_INTERFACE) {
